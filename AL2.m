...
w_acoustic = arrayfun(@(i) corr(RIRs(:, i), RIR_reference), 1:size(RIRs, 2));
...
cvx_begin
    ...
    minimize(norm(x, 1) + lambda * pow_abs((norm(WeightedRIRs * x) - desired_norm), 2))
cvx_end

% Estimated RIR at the new location
estimated_RIR = WeightedRIRs * x;
